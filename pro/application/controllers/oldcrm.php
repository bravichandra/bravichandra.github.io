...
class Crm extends CI_Controller {
...

	{
...

		{
			...
			Am_Lite::getInstance()->checkAccess(array(2,6,5,9,10,14,15,18,28,30,31), 'Restricted access'); //  This array is Updated by Dev@4489 on 23-Oct-2015
			...
			$haveSubscriptions = Am_Lite::getInstance()->haveSubscriptions(array('6','2','9','10','14','15','18','28','5','3','30','31'));//  This array is Updated by Dev@4489 on 17-April-2014
...
//echo "rrrrr";
			...
			if($this->input->post('action')=="save") {
				...
				if(!empty($record['email'])) {
					...
					if (!valid_email($record['email'])) $er['email']="Enter valid email address";
						...
						$email_info = $this->crm->get_acRecord("LOWER(email)='".strtolower($record['email'])."'".$ownq,'contact_id,userid','C');
						if($email_info) {
							//Email existance in Organisation
...

	{
		...
		if($pam3=="edit") {
			...
			$this->_data['opportunities'] = $this->crm->get_all_opportunitys($owner);
...

	{
		...
		if($this->input->post('act')=="questions" && $this->input->post('id')) {
			...
			$questions = $this->campaign->getQualifyQuest($cid);
			...
			if($questions) {
				...
				foreach($questions as $qt) {$qi++;

					echo '<div class="qualify">

							<table cellpadding="0" cellspacing="0" width="100%">

									<tr>

										<td class="box1">'.$qt->value.'</td>
										...
										<td class="box2">
											...
											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="-3" /> -3</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="-2" /> -2</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="-1" /> -1</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" checked="checked" class="optval" value="0" /> 0</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="1" /> 1</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="2" /> 2</label> 

											<label><input type="radio" name="record[section]['.$c.'][qp]['.$qi.']" class="optval" value="3" /> 3</label><br />

											</div>

											<b>Notes</b><br />

											<textarea name="record[section]['.$c.'][task_label]['.$qi.']" class="task_label">'.$qt->value.'</textarea>

											<textarea name="record[section]['.$c.'][task_notes]['.$qi.']" class="task_notes"></textarea>
		...
		if($this->input->post('action')=="save") {
			...
			$qsections = $qualifier_data['sections'];			

			$er = array();

			//if(empty($record['contact'])) $er['contact']="Contact required";

			if(empty($record['contact']) && empty($record['account'])) $er['contact']="Atleast Contact/Account required";
			...
			if(!$er) {
				...
				if($qpoints<>0) {
					...
					if($record['contact']) {
						...
						$where = array('contact'=>$record['contact'],'cat'=>0,'pdate'=>$idate,'rctype'=>'C');
					...
					if($record['account']) {
						...
						$where = array('contact'=>$record['account'],'cat'=>0,'pdate'=>$idate,'rctype'=>'A');
				...
				echo "YES-".($cid1?$cid1:$cid2)."-".($record['contact']?$record['contact']:$record['account']);
	...
	function interaction($action = NULL)
	{
		...
		if($pam3=='contact' && $parent_id) {
			...
			$this->_data['record_title'] = ucfirst($parent_record[user_first].' '.$parent_record[user_last]);
			...
			$this->_data['record_id'] = $parent_id;
			$this->_data['record_title'] = ucfirst($parent_record[account_name]);
		...
		if($this->input->post('action')=="save") {
			...
			*/
			...
			if(!$er) {
				...
				$where = array('contact'=>$record['record_id'],'cat'=>$catg,'pdate'=>$idate,'rctype'=>$pam3=='contact'?'C':'A');
				...
				if($cont_points) {
					...
					$update_data = array(
							...
							'contact'=>$record['record_id'],

							'cat'=>$catg,

							'pdate'=>$idate,

							'rctype'=>$pam3=='contact'?'C':'A'
				...
				if($interactions) {
					...
					foreach($interactions as $ival) {
						...
						$where = array('intr_sno'=>$intr_sno,'intr_date'=>$intr_date,'intr_rctype'=>($pam3=='contact'?'C':'A'),'intr_recid'=>$record['record_id']);
						...
						if($intrdata) {
							...
							$update = array(
								...
								'intr_rctype'=>($pam3=='contact'?'C':'A'),

								'intr_recid'=>$record['record_id'],
...

	{
		...
		if($this->input->post('action')=="save") {
			...
			if(!$er) {
				...
				$where = array('contact'=>$record['record_id'],'cat'=>$record['cat'],'pdate'=>$idate,'rctype'=>$pam3=='contact'?'C':'A');
	...
	public function salesforce($action = NULL) {
		...
		if($_SESSION['instance'] && $_SESSION['access_token'] && $_SESSION['expires']) {			
			...
			if($exp < $pre) {
				...
				header('location: /pro/page1.php?template=yes&crm=y&rc='.$rtype);

			} else $sflogin = 1;

		}

		if(!$sflogin || $action=="login") header('location: /pro/page1.php?template=yes&crm=y&rc='.$rtype);
	...
	public function settings($action="settings") {	
		...
		if($action=="fields" && isset($this->_data['eScripter'])) {
			...
			if($customa) {
				foreach($customa as $ck=>$cv) $alayout_fields[$ck]=array('label'=>$cv,'type'=>'custom');
			}
	...
	public function compose($action=NULL) {
		...
		if($parent_id) {
			...
			if($action && strpos($action,"t")!==FALSE && strlen($action)>1) {
				...
				if(($template_content_id>=96 && $template_content_id<=111) || $template_content_id==161 || $template_content_id==167 || $template_content_id==173 || $template_content_id==179) $this->_data['template_content_id'] = $template_content_id;
			...
			*/		
			...
			try {
			  ...
			  if($Contact_email_info) {			  	
			  	if($contacts_List) {
			  		$schdate = time();
					$schdate = $schdate+5*60; // after 5mins
