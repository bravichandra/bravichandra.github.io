...
class Chrome extends CI_Controller
{
    ...
    public function Templates()
    {
        ...
        if ($alltemplates) {
            ...
            $message .= '<div id=template1>Sales Playbook Email Templates <select id="ss_email_template">';
        ...
        if ($uemail_templates) {
            ...
            $message .= '<div id=template2>User Saved Email Templates <select id="ss_uemail_template">' . $uemail_templates;
            $message .= '</select></div>';
        }
    ...
    {
        log_message('message', 'inside postEmail--------------');
        $userName = $this->input->post('txt_ss_loginname');
        ...
        if ($userId) {
            if ($this->input->post('id')) {
                ...
                if ($res) {
                    $recipient = json_decode($this->input->post('recipient'));
                    ...
                    //create entry in task and add quality point 1
                    $contactInfo = $this->crm->get_contact_by_email($recipientEmail, $userId);
                    if (!empty($contactInfo)) {
                        ...
                        if (!empty($this->input->post('subject'))) {
                            $taskName = $this->input->post('subject');
                        } else {
                            $taskName = 'No Subject';
                        }
                        $taskdata = [
                        ...
                        'task_created' => $idate . ' 00:00:00',
                        'task_modified' => $idate . ' 00:00:00',
                        'task_info' => $this->input->post('messageBoby'),
                        ];
                        $tid = $this->crm->save_task($taskdata, 0);
                        ...
                        'purpoints' => $contpoints['purpoints'] + $pursuitPoints,
                        'taskid' => $tid,
                        ];
                        ...
                        $notificationData = [
                        'userid' => $userId,
                        'info' => 'Email "' . $this->input->post('subject') . '" sent to <a href="' . base_url("crm/contacts/view/$contactId") . '">' . ucfirst($contactInfo[0]->user_first . ' ' . $contactInfo[0]->user_last) . '</a>',
                        'datetime' => date('Y-m-d H:i:s'), ];
                        $this->crm->save_email_notify($notificationData);
        ...
        if ($userId) {
            if ($email && $type && $message) {
                ...
                if ($type == 'emailTrack') {
                    ...
                    $notificationInfo = ' opened email "' . $subject . '"';
                    ...
                    $points = '3';
                    $taskSubject = 'Email link clicked';
                    $taskInfo = 'They clicked the link';
                    $notificationInfo = '</a> clicked the link <a target="_blank" href="' . $ssUrl . '">' . $url . '</a>';
                }
                $contactInfo = $this->crm->get_contact_by_email($email, $userId);
                if (!empty($contactInfo)) {
                    $contactId = $contactInfo[0]->contact_id;
                    ...
                    $notificationData = [
                        'userid' => $userId,
                        'info' => '<a href="' . base_url("crm/contacts/view/$contactId") . '">' . ucfirst($contactInfo[0]->user_first . ' ' . $contactInfo[0]->user_last) . '</a>' . $notificationInfo,
                        'datetime' => date('Y-m-d H:i:s'), ];
                    $this->crm->save_email_notify($notificationData);
                    $output = ['result' => true, 'message' => 'Points saved successfully'];
                } else {
    ...
    public function saveEmailCopy()
    {
        $userName = $this->input->post('txt_ss_loginname');

        $gmailMessageId = $this->input->post('gmail_message_id');
        $gmailThreadId = $this->input->post('gmail_thread_id');
        $email = $this->input->post('email');
        $subject = $this->input->post('subject');
        $messageBoby = $this->input->post('message_boby');
        ...
        if ($userId) {
            if ($email && $gmailMessageId && $gmailThreadId && $subject) {
                $type = $this->input->post('type');
                $purPoints = 0;
                if ($type == 'reply') {
                    $points = 3;
                    $taskSubject = 'Contact replied';
                    $notificationInfo = ' replied to your email "' . $subject . '"';
                } elseif ($type == 'no reply') {
                    $points = 5;
                    $taskSubject = 'Contact emailed';
                    $notificationInfo = ' emailed you "' . $subject . '"';
                }

                $res = $this->crm->get_task_by_gmail_ids($gmailMessageId, $gmailThreadId, $userId);
                if (!empty($res)) {
                    $output = ['result' => false, 'data' => json_encode($res), 'message' => 'Email already saved in database'];
                } else {
                    $contactInfo = $this->crm->get_contact_by_email($email, $userId);
                    if (!empty($contactInfo)) {
                        $contactId = $contactInfo[0]->contact_id;
                    ...
                    'gmail_thread_id' => $gmailThreadId,
                        ...
                        $output = ['result' => false, 'message' => 'Contact does not exists in database'];
                    }
                }
            } else {
                $output = ['result' => false, 'message' => 'Invalid input'];
            }
        } else {
            $output = ['result' => false, 'message' => 'Invalid user'];
        }
