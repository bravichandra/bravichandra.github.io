...
class Product_model extends CI_Model 
{
	...
	public function create_product($product_name = NULL)
	{
		...
		if($product_name == NULL) {
			$create_product = array('user_id' => $user_id,'product_name' => '[your product]');
		}else{
		...
		/* echo $this->db->last_query();
		 die(); */
		$create_product_data = array('product_id' => $product_id,'field_type' => 'P_Q1','value' => $product_name?$product_name:'[your product]');		
		$this->db->insert($this->_table_product_data,$create_product_data);
		
		
		$Product_desc_data = array('product_id' => $product_id,'field_type' => 'P_Desc','value' => '[Product Description]');		
	...
	public function createNewCampaign($campaign_name='default-campaign',$organization='businesses',$prod_id=0)
	{
	    $user_id = $_SESSION['ss_user_id'];	
		...
		//if no product create new product
		if(!$prod_id) $prod_id = $this->create_product();
	...
	public function createSalesQuestion($campaign_id,$tech_pain_id,$qus_id,$type)
	{
		$sales_question_data = array('campaign' => $campaign_id ,'field_type' => $type,'question' => 'Answer'.$qus_id,'qus_id' => $qus_id);
		$this->db->insert('sales_question',$sales_question_data);
		$sales_question_id = $this->db->insert_id();
				...
				foreach($pre_name  as $primary_id => $key_value)
				{
					foreach($key_value as $key => $value)
					{
						...
						if($primary_id) {
							...
							//product data key value not exists
							$this->db->where('field_type',$key);
							$this->db->where('product_id',$editproduct_id);
							$record = $this->db->get($realtablename);
							if($record->num_rows == 0)
							{
								$product_data = array('product_id' => $editproduct_id, 'field_type' => $key ,'value' => $value);
								$this->db->insert($realtablename,$product_data);
							}
						}
						// echo $this->db->last_query();
						//echo $this->db->last_query();
	 ...
	 *   update product data
	 */
	public function update_product_data($product_id,$evalue,$key)
	{ 
		$data = array('value' => $evalue);
		$this->db->where('product_id',$product_id);
		$this->db->where('field_type','P_Q1');
		return $this->db->update($this->_table_product_data,$data);
	}
	...
	public function vit_ignore_save($vit_igdata,$campid,$igtool='V',$iqtype='t')
	{			
		if(!empty($vit_igdata))
		{		
			//delete existance
			...
			$this->db->where('iqtooltype',$igtool);
			$this->db->delete($this->_table_vit_ignore);					
			//insert record
			$updaterecord = array('campaign_id' => $campid,'qid' => (int)$vit_igdata['qid'],'iqlist' => $vit_igdata['ignore'],'iqtype' => $iqtype,'iqtooltype' => $igtool);
			$this->db->insert($this->_table_vit_ignore,$updaterecord);
		}
	}
	//VALUE IDENTIFIER TOOL: get ignore questions
	public function vit_ignore_get($campid,$igtool='V',$iqtype='t')
	{
