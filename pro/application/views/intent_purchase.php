...
<form  action="" method="post" onsubmit="step2();">
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin:20px 0px !important;">
                <tr>
                    ...
                    <td align="left">
                         <input type="hidden" name="squestions_title_id" value="<?php echo $tabtitles[4]->id;  ?>" class="qorder"/>
                            <input type="hidden" name='squestions_type' value="<?php echo $tabtitles[4]->qb_type;  ?>" />
                            <input type="text" class="cat_cus_title qorder" placeholder="Decision Authority" value="<?php echo $tabtitles[4]->title;  ?>" name="squestions_title"/>
                    </td>
                </tr>
        </table>
        <div class="widget tableTabs"> 
             <div class="tab_container">
                  <div id="ttab1" class="tab_content">
                        <table cellpadding="0" cellspacing="0" width="100%" class="tDefault" id="tq_table_1">
                           <tbody id="SalesQuestionTbody5">
                                  <?php $i=0; $q_cols_counts = 0;$q_cols = array(); ?>
                                  <?php if($intent_purchase) {$q_cols_counts += count($intent_purchase);?>
                                  ...
                                  <?php 
                                    foreach($intent_purchase as $sintechintpur) { 
                                    $i++;
                                  ?>
                                  <tr class="SalesQuestionTrClass" id="sque_<?php echo $sintechintpur->id; ?>">
                                  	 <input type="hidden" name="qid[]" value="<?php echo $sintechintpur->id; ?>" />
                                      <td class="no-border noBorderB" width="70%" align="left" valign="middle" id="ansListh<?php echo $sintechintpur->id;?>">
                                          <span class="TextColor">Question <?php echo $i; ?></span>
                                       </td>
                                       <td class="no-border noBorderB" style="width: 32%;">
                                           <div class="grid5"> 
                                                 <textarea class="validate[required] gans<?php echo $sintechintpur->id;?>" style="width:500px;" id="QQ_7" name="need_questions[]"
                                                  cols="" rows=""><?php echo $sintechintpur->question; ?></textarea>
                                                <div style="margin-top: 20px;">
                                                    <div> <input type="checkbox" value="1" <?php if(!$sintechintpur->visible) echo 'checked="checked"'; ?>   
                                                    onchange="updateNeedDisplay(<?php echo $sintechintpur->id; ?>,this,0);" /> Do not display answer in templates</div>
                                                    <div style="padding-top:10px;"> 
                                                        <input type="checkbox" value="1" <?php if($sintechintpur->highlight) echo 'checked="checked"'; ?>   onchange="updateNeedHighlight(<?php echo $sintechintpur->id; ?>,this,0);" />
                                                        Highlight answer in sales scripts
                                                    </div><br>
                                                    <div align="center"><a href="javascript:void(0);" class="buttonM bRed" 
                                                    onClick="show_fquestions(<?php echo $sintechintpur->id;?>,0)">Add a Follow-Up Question</a></div>
                                                </div>
                                           </div>
                                       </td>
                                       <td class="no-border noBorderB">
                                           <div class="grid5">
                                                <div align="center"><span class="ui-icon ui-icon-closethick" id="hide_22476" onclick="hide_box_status2('<?php echo $sintechintpur->id;?>');">X</span></div>
                                           </div>
                                      </td>
                                      <td class="noBorderB">
                                          <input type="text" name="qus_id[]" class="qorder" value="<?php echo $sintechintpur->qus_id; ?>" 
                                          style="height:20px; border:1px solid #999999; text-align:center;" size="2" 
                                          onchange="updateSorder2(<?php echo $sintechintpur->id; ?>,this.value,this);">
                                      </td>
                                  </tr>
                                  <?php 
                                        //display sales responses
                                        sales_resplist_htmledit_view($sintechintpur->id,$campaign_info->campaign_id);
                                    ?> 
            ...
            <div style="background:#ffffff; padding:0px 8px 8px;"> 
                <div align="right">
                    <input type="button" onclick="DynamicAddRowSalesQuestion(this,'tab5')" class="buttonM bBlack" value="Add a Question" style="margin-top:10px;color:white !important;">	
                </div>
                <div class="fluid" style="margin-top:-5px; float:left;">
                        <input style="margin-top: -25px; margin-right: 10px" type="submit" class="buttonM bBlue" name="form_submit5" value="Save">
