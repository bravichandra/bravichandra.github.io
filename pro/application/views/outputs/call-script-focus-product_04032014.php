...
<title>Call Script - Product Focus</title>
</head>
...
<h1 align="center">Call Script &#45; Product Focus</h1>
<!-- <p class="topTxt"><strong>Description:</strong> This is a call script and that goes quickly to the subject of pain and tries to identify if the prospect is having any pain in the areas that these products can resolve.</p> -->    
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="output">
  ...
  <tr style="border-bottom:1px solid #000;">
    ...
    <td style="width:80%;">
    Option A: 
    <span class="dynamic_value edit_area" id="edit_<?php echo $user_id;?>_<?php echo (isset($sales_process['0']->user_data_id) ? $sales_process['0']->user_data_id : Null);?>__close__tud"><?php echo (!empty($sales_process[0]->value) ? $sales_process[0]->value : $this->config->item('message'));?></span><br/> 
    Option B:  
    <span class="dynamic_value edit_area" id="edit_<?php echo $user_id;?>_<?php echo (isset($sales_process['2']->user_data_id) ? $sales_process['2']->user_data_id : Null);?>__close__tud"><?php echo (!empty($sales_process[2]->value) ? $sales_process[2]->value : $this->config->item('message'));?></span></td>
  </tr>
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>The purpose for my call is:<br/><span class="red-area">(Share any of the below as appropriate as you try to trigger interest)</span><br><br>
  	...
  	<tr>
  		<td colspan="3" class="border">&nbsp;</td></tr>
	  <tr>
	    <td class="td-pad-b"><strong>Disqualify Statement</strong></td>
	    <td>&nbsp;</td>
	    <td> I actually don't know if you need what our services provide so I just had a question or two. </td>
	</tr>
  <tr><td colspan="3" class="border">&nbsp;</td></tr>
  <tr>
						...
						<ul>
							<?php if (isset($products)):?>
								<?php foreach ($products as $product):?>
										...
										<?php if($counter == 1):?>
											<li><span class="<?php echo (!empty($technical->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $technical->id;?>__tech_qualify__tpd"><?php echo $technical->value;?></span></li>
											<li><span class="<?php echo (!empty($detail_tech_data[0]->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $detail_tech_data[0]->id;?>__desc_tech_qualify__tpd"><?php echo $detail_tech_data[0]->value;?></span></li>
										<?php else:?>
											<li><span class="<?php echo (!empty($technical->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $technical->id;?>__tech_qualify__tpd"><?php echo $technical->value;?></span></li>
										<?php endif;?>
								...
								<?php foreach ($products as $product):?>
										...
										<?php foreach ($single_technical_data as $single_technical):?>
											<li><span class="<?php echo (!empty($single_technical->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $single_technical->id;?>__single_tech_qualify__tpd"><?php echo $single_technical->value;?></span></li>
										<?php endforeach;?>
										<?php endif?>
								<?php endforeach;?>
							<?php endif;?> 
							...
							<?php if (isset($products)):?>
										<?php foreach ($products as $product):?>
											...
											<?php if(isset($bus_qualify_data)):?>
													<?php $counter=1; foreach ($bus_qualify_data as $business):?>
														<?php if($counter == 1):?>
															<li><span class="<?php echo (!empty($business->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $business->id;?>__bus_qualify__tpd"><?php echo $business->value;?></span></li>
															<li><span class="<?php echo (!empty($detail_bus_data[0]->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $detail_bus_data[0]->id;?>__desc_bus_qualify__tpd"><?php echo $detail_bus_data[0]->value;?></span></li>
														<?php else:?>
															<li><span class="<?php echo (!empty($business->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $business->id;?>__bus_qualify__tpd"><?php echo $business->value;?></span></li>
														<?php endif;?>
													<?php $counter++; endforeach;?>
									...
									<?php foreach ($products as $product):?>
										...
										<?php foreach ($single_business_data as $single_business):?>
											<li><span class="<?php echo (!empty($single_business->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $single_business->id;?>__single_bus_qualify__tpd"><?php echo $single_business->value;?></span></li>
										<?php endforeach;?>
										<?php endif?>
								<?php endforeach;?>
								<?php endif;?> 
	...
	<strong>Personal Questions:</strong>
									...
									<?php foreach ($products as $product):?>
										...
										<?php if(isset($pers_qualify_data)):?>
											<?php $counter=1; foreach ($pers_qualify_data as $personal):?>
												<?php if($counter == 1):?>
													<li><span class="<?php echo (!empty($personal->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $personal->id;?>__pers_qualify__tpd"><?php echo $personal->value;?></span></li>
													<li><span class="<?php echo (!empty($detail_pers_data[0]->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $detail_pers_data[0]->id;?>__desc_pers_qualify__tpd"><?php echo $detail_pers_data[0]->value;?></span></li>
												<?php else:?>
													<li><span class="<?php echo (!empty($personal->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $personal->id;?>__pers_qualify__tpd"><?php echo $personal->value;?></span></li>
												<?php endif;?>
											<?php $counter++; endforeach;?>
						...
						<?php foreach ($products as $product):?>
										...
										<?php foreach ($single_personal_data as $single_personal):?>
											<li><span class="<?php echo (!empty($single_personal->value) ? 'dynamic_value edit_area' : '');?>" id="edit_<?php echo $product->product_id;?>_<?php echo $single_personal->id;?>__single_pers_qualify__tpd"><?php echo $single_personal->value;?></span></li>
										<?php endforeach;?>
										<?php endif?>
								<?php endforeach;?>
							<?php endif;?> 
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>When we talk with other [insert prospect's title], we have noticed that they often express challenges with:<br/><span class="red-area">(Share any of the below pain examples)</span><br/>
	    	<br>
	        ...
	        <p>Are you concerned with any of those?</p><br>
	</td>
  </tr>
  ...
  <tr>
    ...
    <?php if (isset($credibilities)):?>
       		<?php $i = 1; foreach ($credibilities as $credibility):?>
       				...
       				?>
	    	<br><strong>Name Drop #<?php echo $i;?>:</strong>
	    	<br>We worked with <span class="dynamic_value edit_area" id="edit_<?php echo $credibility->c_id;?>_<?php echo $customer_id;?>__customer__tcd"><?php echo $customer_value;?></span> and provided 
	    	...
	    	<br/>This helped them to <span class="dynamic_value edit_area" id="edit_<?php echo $credibility->c_id;?>_<?php echo $provided_id;?>__provided__tcd"><?php echo $provided_value;?></span>, which led to 
