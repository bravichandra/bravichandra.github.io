...
<title><?php echo isset($uCustTemplate['template_title'])?$uCustTemplate['template_title']:'Call Script – Qualify Focus'; ?></title>
</head>
<body>
...
<?php if(!$partid && $action!='download2') { ?>	
	<h1 align="center"><?php echo isset($uCustTemplate['template_title'])?$uCustTemplate['template_title']:'Call Script &#45; Qualify Focus'; ?></h1>
	<p class="topTxt">
	...
	{
		include_once('interactive/introduction_data.php');
	 	$temp = end(explode('/',current_url()));
	...
	else if($partid == 2 && $partid <= $lastid)
	{
	 ?>
     	<h1 class="sub-dev-head"><?php echo $parts[$partid]['title'];?></h1>
        <div class="scroll-bar_sf"><?php $content_id=3;include('custom_content/custom_etemplate_data.php');?></div>
     <?php
	...
	{
		include_once('interactive/pre_qualify_data.php');
	 	$temp = end(explode('/',current_url()));
		$back_step = str_replace($temp,$parts[$partid-1]['name'],current_url());
		$front_step = str_replace($temp,$parts[$partid+1]['name'],current_url());
		$fav_step = str_replace($temp,'close',current_url());
	...
	{
		include_once('interactive/common_problems_data.php');
	 	$temp = end(explode('/',current_url()));
		$back_step = str_replace($temp,$parts[$partid-1]['name'],current_url());
		$front_step = str_replace($temp,$parts[$partid+1]['name'],current_url());
		$fav_step = str_replace($temp,'close',current_url());
	...
	{
	 	include_once('interactive/about_us_data.php'); 	
	 	$temp = end(explode('/',current_url()));
		$back_step = str_replace($temp,$parts[$partid-1]['name'],current_url());
		$front_step = str_replace($temp,$parts[$partid+1]['name'],current_url());
		$fav_step = str_replace($temp,'close',current_url());
	...
	{
		include_once('interactive/close_data.php');
	 	$temp = end(explode('/',current_url()));
		$back_step = str_replace($temp,$parts[$partid-1]['name'],current_url());
