...
<style>
.pt10 a {color:black;}.align-center{text-align: center;}.main-wrapper div.wrapper:nth-child(odd) > :first-child {background: #B9B9BD;}.main-wrapper div.wrapper:nth-child(even) > :first-child {background: #B9B9BD;}/*.main-wrapper div.wrapper:nth-child(odd) {  color: #ccc;}*/
...
<div id="content">
				...
				<?php // if (!empty($all_requests) or !empty($all_receiver_requests)): ?>		
				  ...
				  <div class="fluid sales_basis">
						<div class="grid6" style="width:100%;">
							<div class="widget" style="padding-right:20px;">
								<div class="body">
                    			<h1 style="margin-left:10px; margin-bottom:10px;" class="pt10">Sales Prospecting Basics</h1>
                                <table cellpadding="0" cellspacing="0" width="100%" class="tDefault">
                                    <tbody>
                                            <?php /*?><tr style="border:none;">
                                            	<td class="no-border"><h2 class="pt10">Module</h2></td>
                                                <td class="no-border"><h2 class="pt10">Topic</h2></td>
                                                <td class="no-border"><h2 class="pt10">Order</h2></td>
                                                <td class="no-border" style="width:100px;"><h2 class="pt10">Video Watched</h2></td>
                                                <td class="no-border"><h2 class="pt10">Quiz</h2></td>
                                                <td class="no-border" style="width:100px;"><h2 class="pt10">Status</h2></td>
                                            </tr><?php */?>
                                            <?php
												$i=0;
												foreach($videos_list as $key=>$value)
												{
												$quizstatus = $this->crm->spbquizstatus($value->id,'');
	...
	<script type="text/javascript">
			...
			function watchTraining(dis,status,vid) {
			 ajrequest = $.ajax({
			 type: "POST",
			 url: "<?php echo base_url('scripts/salesProspectingBasics');?>", 
			 cache: false,
			 dataType: 'json',
			 data: 'ctype='+dis+'&status='+status+'&vid='+vid+'&vid='+vid,
			}).done(function( resp ) {
           // alert(resp.tstatus);
            $(".sales_basis").hide();
            $(".sales_video").show();
			...
			$("#mark_docs").html("<input type='checkbox' id='"+status+"' name='"+status+"' class='mstatus' onchange='updateBasicDetails(this,0);'/> Mark to document that you have watched training module");
			 ...
			 if(resp.spbquizstatus==5) {
			  $("#finalQuiz").html('<input type="button" class="buttonM bGreen" name="launch" value="Quiz Complete" onclick="takeQuiz(1);">');
			 }
			 else
			 {
			 	 $("#finalQuiz").html('<input type="button" class="buttonM bGreen" name="launch" value="TAKE Quiz" onclick="takeQuiz(0);">');	
			 }
			})
	  }
      function updateBasicDetails(dis,qp) {
            ...
            $.ajax({
                type : 'POST',
                url : '<?php echo base_url('crm/salesprospectingdetails');?>',
                data: 'val='+vis+'&qp='+qp+'&status='+status,
                cache: false,
                dataType: 'json',
                success: function(responce)
                {
                    location.replace("sales-prospecting-basics"); 
                }
            });
		...
		function takeQuiz(dis,$vid)
		{
			...
			$.ajax({
                type : 'POST',
                url : '<?php echo base_url('crm/spbquestions');?>',
		...
		function checkQuizStatus()
		{
			...
			$.ajax({
                type : 'POST',
                url : '<?php echo base_url('crm/spbquizstatus');?>',
                data: 'vid='+vid,
                cache: false,
                dataType: 'json',
                success: function(responce)
		...
		function submitQuiz(obj)
		{
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url('crm/submitquestions');?>",
				dataType: "text",
				async:false,
