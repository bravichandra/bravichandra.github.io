...
function addRow(table_id,type,p_id) 
{
	...
	$.ajax({
		  ...
		  success: function(response)
		  { 
                      if(response.tech_count > 2)
			{
			  //$("#show-tech").show();
			  //$("#show-tech-heading").show();
			}
                        if(response.bus_count > 2)
			{
			  //$("#show-bus").show();
			  //$("#show-tech-heading").show();
			}
                        if(response.pers_count > 2)
			{
	...
	$.ajax({
		  ...
		  success: function(response)
		  {
		    ...
		    newRow.innerHTML =
		                ...
		                +'<td class="no-border">'+lable+'<textarea id='+id+' class="validate[required] dynamicTxt" style="width:350px;" name="'+ field_name +'" cols="" rows=""></textarea><div align="center" class="TextColorH">Answer Checker</div><div align="center"><span class="dynamicTxt_'+ id +' TextColor"></span></div></td>\n'+ '\n'
...
<script>
    ...
    //$("#show-tech-heading-summary").hide();
    //$("#show-bus-heading-summary").hide();
    //$("#show-pers-heading-summary").hide();
    ...
    if(TechTrRowCount > 0)
    {
      $("#TechRadioId").attr ("checked", "checked");
      $("#TechnicalDiv").show();      
    }
    else
    {
    ...
    if(TechTrRowCount > 2)
    {
        $("#show-tech-heading-summary").show();
        $("#show-tech").show();
    }else{
        $("#show-tech-heading-summary").hide();
        $("#show-tech").hide();        
    ...
    if(BusTrRowCount > 0)
    {
      $("#BusradioId").attr ("checked", "checked");
      $("#BusinessDiv").show();
      $("#BusinessHeading").show();
      $("#BusYesNoQuestion").show();
    }
    ...
    {
      $("#BusYesNoQuestion").hide();
      $("#BusinessHeading").hide(); 
      $("#BusinessDiv").hide();  
    }
            
    if(BusTrRowCount > 2)
    {
        $("#show-bus-heading-summary").show();
        $("#show-bus").show();
    }else{
        $("#show-bus-heading-summary").hide();
        $("#show-bus").hide();        
    ...
    if(PersTrRowCount > 0)
    {
      $("#PersradioId").attr ("checked", "checked");
      $("#PersonalDiv").show();
      $("#PersonalHeading").show();
      $("#PersYesNoQuestion").show();
    }
    else
    {
      ...
      $("#PersonalDiv").hide(); 
      $("#PersYesNoQuestion").hide();
    }
    if(PersTrRowCount > 2)
    {
        $("#show-pers-heading-summary").show();
        $("#show-pers").show();
    }else{
        $("#show-pers-heading-summary").hide();
        $("#show-pers").hide();        
    ...
    */
    ...
    <?php
    $SessionId = $this->session->userdata('ss_session_id');
    $VerifyQuery = $this->db->query("SELECT * from tbl_value where SessionId='$SessionId'");
    if($VerifyQuery->num_rows() > 0)
    {?>
            
        <?php
        $Result = $VerifyQuery->row();
        if($Result->TechQues == Yes)
        {   ?>
            $("#TechRadioId").attr ("checked", "checked");
            $("#TechnicalDiv").show();
...
function DynamicAddRow(table_id,type,p_id,OnClickVal,pro_1_value,CurrentTRid)
{   
    if(type == 'T')
    {
      var NumberOne = 1;
...
function AddNewRow(table_id,type,p_id,table_rowid,OnClickVal,pro_1_value)
{
    ...
    if(type == 'B')
    {
      ...
      if(OnClickVal == "Yes")
        {   
            var BusTrRowCount = $('#'+table_id+' tr').length;
            var LastBusTrId = $('#'+table_id+' tr:visible:last').prev('tr').attr("id");
            ...
            if(BusTrRowCount > 0)
            {
                ...
                {
                    AddingDynamicRow(table_id,"B",p_id,NextRowId,pro_1_value);
                }
                */
            }
            else
            {
                AddingDynamicRow(table_id,"B",p_id,"BusTR1",pro_1_value);
            }
        }
    }
    ...
    if(type == 'P')
    {
      ...
      if(OnClickVal == "Yes")
        {   
            var PersTrRowCount = $('#'+table_id+' tr').length;
            var LastPersTrId = $('#'+table_id+' tr:visible:last').prev('tr').attr("id");
            ...
            if(PersTrRowCount > 0)
            {
                ...
                {
                    AddingDynamicRow(table_id,"P",p_id,NextRowId,pro_1_value);
                    $('#'+RadioTrId).removeAttr('style');
            ...
            {
                AddingDynamicRow(table_id,"P",p_id,"PersTR1",pro_1_value);
                $('#'+RadioTrId).removeAttr('style');
            }
        }
    }
		  ...
		  success: function(response)
		  {
			...
			else
			{
				  ...
				  var p_value = '[personal value 1]';
                    ...
                    if(type == 'T')
                    {
                        var ExpVal = NewTrName.split("TechTR");
                        var LastDigit = parseInt(ExpVal[1]);
                        ...
                        RadioTrId = "TechTRRadio"+LastDigit;
                        RowTitle = "technical";
		    ...
		    if(type == 'B')
		    {
                       var ExpVal = NewTrName.split("BusTR");
                       var LastDigit = parseInt(ExpVal[1]);
		       ...
		       var lable = '<div align="center" class="TextColorH">Business Value</div>';
                       ...
                       RadioTrId = "BusTRRadio"+LastDigit;
                       RowTitle = "business";
		    }

		    if(type == 'P')
		    {
                       var ExpVal = NewTrName.split("PersTR");
                       var LastDigit = parseInt(ExpVal[1]);
		       ...
		       var lable = '<div align="center" class="TextColorH">Personal Value</div>';
                       ...
                       RadioTrId = "PersTRRadio"+LastDigit;
                       RowTitle = "personal";
		    }
		    ...
		    var deladd = "delSingleAdditionalQus('"+response.id+"','value');";  
                    ...
                    var RadioTrInnerHTML = "<td colspan='5'>"+
                        "Does <a title'' href='<?php echo base_url();?>index.php/step/product'> "+pro_1_value+" </a>deliver any other "+RowTitle+" improvements?<br/>"+
                        "<input type='radio' name='"+BtnName+"' id='"+BtnId+"' class='"+BtnClass+"' onclick='DynamicAddRow(\""+table_id+"\",\""+type+"\",\""+p_id+"\",this.value,\""+pro_1_val+"\",\""+RadioTrId+"\");' value='Yes'>Yes </br>"+
                        "<input type='radio' name='"+BtnName+"' id='"+BtnId+"' class='"+BtnClass+"' onclick='DynamicAddRow(\""+table_id+"\",\""+type+"\",\""+p_id+"\",this.value,\""+pro_1_val+"\",\""+RadioTrId+"\");' value='No'>No </br>"+
                        "</td>";
		    ...
		    if(type == 'B' || type == 'P')
		    {
				if(type == 'B' && product_data[p_id]['business']['first_time'] == true)
				{
                                        ...
                                        '<td class="no-border" style="width: 13em;">When <span class="TextColor"><?php  echo (!empty($custome_fields['1']->value) ? $custome_fields['1']->value : 'businesses');?></span> are able to see the improvement of</td>'

                                        +'<td class="grid5 no-border" style="width: 20em;" ><span class="TextColor dynamicFillTecArea_'+p_id+'"></span></td>'

                                        +'<td class="no-border" style="width: 38em;">a business improvement (revenue, costs, services) that they can see is</td>'

                                        +'<div class="grid5"><div align="center" class="TextColorH">Business Value</div><textarea class="validate[required] vs_'+table_id+' dynamicTxt '+TextClass+' dynamicFillBus_'+p_id+'" style="width:350px;margin-left: 12px;margin-bottom: 9px;" name="'+ field_name +'" id='+id+' cols="" rows="" '+KeyUPFun+'>[business value 1]</textarea><div align="center" class="TextColorH">Answer Checker</div><div align="center">We help <span class="dynamicFillTecArea TextColor"><?php echo (!empty($custome_fields['1']->value) ? $custome_fields['1']->value : 'businesses');?></span> to <span class="dynamicTxt_'+ id +' TextColor"></span>.</div></td><td class="no-border"><div class="grid4"><div id="1" data-icon="&#xe090;" class="dialog_open fs1 iconb"></div></div></td>';
				...
				if(type == 'P' && product_data[p_id]['personal']['first_time'] == true)
				{
					rowInnerHTML = 
                                            ...
                                            '<td class="no-border" style="width: 13em;">When <span class="TextColor"><?php  echo (!empty($custome_fields['1']->value) ? $custome_fields['1']->value : 'businesses');?></span> are able to see the improvements of</td>'

                                            +'<td class="grid5 no-border" style="width: 20em;" ><span class="TextColor dynamicFillTecArea_'+p_id+'"></span> and <span class="TextColor dynamicFillBusArea_'+p_id+'"></span></td>'

                                            +'<td class="no-border" style="width: 38em;"> a personal improvement (career, compensation, work environment) that they can see is</td>'

                                            +'<div class="grid5"><div align="center" class="TextColorH">Personal Value</div><textarea class="validate[required] dynamicTxt vsp_'+table_id+' '+TextClass+'" style="width:350px;margin-left: 12px;margin-bottom: 9px;" name="'+ field_name +'" id='+id+' cols="" rows="" '+KeyUPFun+'>[personal value 1]</textarea><div align="center" class="TextColorH">Answer Checker</div><div align="center">We help <span class="dynamicFillTecArea TextColor"><?php echo (!empty($custome_fields['1']->value) ? $custome_fields['1']->value : 'businesses');?></span> to <span class="dynamicTxt_'+ id +' TextColor"></span>.</div></td><td class="no-border"><div class="grid4"><div id="1" data-icon="&#xe090;" class="dialog_open fs1 iconb"></div></div></td>';
				    ...
				    if(typeof RUN_AUTOFILL_METHOD != "undefined"){
				    	autoFill(p_id);
                                    ...
                                    if(type == 'T')
                                    {
                                        ...
                                        $('#'+table_id+' tr:visible').removeAttr('style');
