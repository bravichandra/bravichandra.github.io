...
<link href="<?php echo base_url();?>css/styles.css?<?php echo time(); ?>" rel="stylesheet" type="text/css" />
...
<script language="JavaScript" type="text/javascript" src="<?php echo base_url(); ?>js/scripts.js?<?php echo rand()?>"></script> 
	...
	$.ajax({
		  ...
		  success: function(response)
		  {
		    ...
		    newRow1.innerHTML =
		                ...
		                +'<td class="no-border"><div class="grid5">'+lable+'<textarea id="P'+p_id+'_'+response.d_id+'" class="validate[required] dynamicTxt" style="width:350px;" name="'+ field_name +'" cols="" rows="" >'+qus_text+'</textarea>'
                                +'<div align="center" class="TextColorH">Answer Checker</div>'
                                +'<div align="center">'
                                +'A lot of <span class="dynamicFillTecArea TextColor"><?php echo (!empty($custome_fields['1']->value) ? $custome_fields['1']->value : 'businesses');?></span> express concerns with <span class="dynamicTxt_P'+p_id+'_'+response.d_id+' TextColor">'+qus_text+'</span>.'
                                +'</div>'
                                +'</div></td>'
...
function deleteSession(session_id,status)
{
	...
	if (answer){
		$.ajax({
			  ...
			  {
			    if(response==0){
				  alert('This product cannot be delete because it is associated with a campaign. Either delete the associated campaign or assign the campaign to a different product in order to delete.');
				}else{
					//By Dev@4489
...
function saveToMyFolderCredibility(unique_id,crdName,userId)
{
	$.ajax({
		url:  BASE_URL+'campaign/savetomycredibility',
		type: 'POST',
		data: {crd_id : unique_id, name: crdName, user_id : userId},
		success: function (response){
				console.log(response);
				//location.reload(true);
			    location.reload(true);
			  // window.location.href = 'http://salesscripter.com/beta/folder/my-folder';
		}
	});
}
...
<?php /*<script type="text/javascript" src="<?php echo base_url();?>js/plugins/tables/jquery.dataTables.js"></script>*/?>
...
<script type="text/javascript" src="<?php echo base_url();?>js/files/functions.js?rand=<?php echo time();?>"></script>
<!--
...
-->
<script type="text/javascript" src="<?php echo base_url();?>js/nav-collapse.js"></script>
</head>
<body>
